...
## Агенты (v5 — 12 ролей)
```
CMO → Head of Strategy (Researcher, Tactic Writer, Audience Analyst)
    → Head of Content (ContentOps, Merch Planner)
    → Head of Growth (Outreach Writer, Growth Scout)
    → Critic (независимый)
```
...
### Орг-структура и система (REFLECT — 3 апреля 2026)

**Пользователь строит ПЛАТФОРМУ, не фестиваль.** Не предлагай операционные задачи (письма, посты) когда нужна архитектура. Различай: «построить систему» ≠ «выполнить задачу».

**При смене сессии/окна — первым делом изучи git log + новые файлы.** Не гадай что изменилось — проверь.

**Не утверждай факты о системе без curl/grep.** «8 агентов» оказалось ≠ реальности. Всегда проверяй актуальное состояние.

**ТОЛЬКО добавлять, НЕ трогать работающее.** 56 done issues, 14 скиллов, Google Sheets — при расширении не ломай. +1 строка delegation hint максимум.

**Стандарты ЧиП определяют архитектуру.** Если стандарт 2.5 требует AJTBD в каждом брифе — нужен отдельный агент (Audience Analyst), а не скилл.

### Дашборд (REFLECT — 3 апреля 2026)

**Дашборд для Жени, не для разработчика.** Убирай внутреннюю кухню (Черновики 35/100, Разделы 19/13). Только метрики, влияющие на решения: билеты, бюджет, амбассадоры, охват.

**Весь UI — на русском.** AWARENESS → УЗНАВАЕМОСТЬ. Никаких английских терминов в пользовательском интерфейсе.

**Полная проверка UI после серии правок.** Не показывай пользователю промежуточные состояния с битой кодировкой и мусором.

### Git и песочница (REFLECT — 3 апреля 2026)

**Git в песочнице Claude ≠ git в терминале пользователя.** `git init` + `git remote add` в sandbox не видно в Terminal.app. Давай одну полную команду для терминала, не серию попыток.

**dashboard-server.js запускать из корня проекта**, не из dashboard/. Путь: `cd ~/Desktop/Extreme_festival && node dashboard-server.js`.

### Качество артефактов (REFLECT — 3 апреля 2026, критическое)

**Каждая задача агенту = Prompt Contract с acceptance criteria.** Не «напиши бриф», а:
- Формат: md, 5-15 страниц
- Содержание: 5 блоков по стандарту ЧиП 2.5
- Ограничения: без слов «уникальный/профессиональный»
- Тест готовности: подрядчик может начать работу без звонка

**Артефакт actionable = copy-paste за 30 секунд.** Если организатору нужно «адаптировать», «дополнить», «переформулировать» — артефакт не готов.

**Information Degradation** — goal размывается в цепочке CMO → Head → Specialist. Решение: оригинальный запрос пользователя передаётся Specialist дословно, не пересказывается Head-агентом.
